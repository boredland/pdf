 ...
 * @returns {Uint8Array}
 */
export function encode_jbig2_generic(packed_bits, width, height) {
    const ptr0 = passArray8ToWasm0(packed_bits, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.encode_jbig2_generic(ptr0, len0, width, height);
    if (ret[3]) {
        throw takeFromExternrefTable0(ret[2]);
    }
    var v2 = getArrayU8FromWasm0(ret[0], ret[1]).slice();
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('jbig2_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
